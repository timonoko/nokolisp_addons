...
'(%Z%%M% %R%.%L%.%B%.%S% %E% %Y%)
'(MIKKO-3 (25 / 11 - 2003) (9 : 54 : 7 57))
(defq *package* SEVENSEG)
...
(defun tulostaseg ()
 ...
 (for
  ...
  (for
   (x 32 126)
   (setq y (assoc x segmentit))
   (print 'RETLW)
   (sp)
   (hex t)
   (print
    (if y (if (zerop b) (cadr y) (caddr y)) 0))
   (hex)
   (print ';)
   (sp)
   (printc x)
...
(defq SEVENSEG
 (tulostaseg esimerkki any-sexp teksti demo nayta-kirjain koodaa-kirjaimet
  kirjaimet segment-17 segment-17-byte RTS DTR taajuus standard-init
  send-byte byte-received? initialize-serial-port SEVENSEG))
